class ShopPage {
    ...
    clickProductName(num) {
        cy.get('.inventory_item_name').eq(num).parent('a').then((link)=>{
    ...
    clickProductImg(num) {
        cy.get('.inventory_item_img').eq(num).parent('a').then((link)=>{
    ...
    sortProductZA() {
        ...
        cy.get('.inventory_item_img').parent('a').children('img').then((name)=> {
